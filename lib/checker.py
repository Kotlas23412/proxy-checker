...
def check_key_e2e(vless_line: str, debug: bool = False, cache: Optional[dict] = None) -> tuple[str, bool, Optional[dict]]:
    ...
    # Ограничиваем одновременно активные xray-процессы, чтобы параллельность не провоцировала false negative
    # из-за перегруза (CPU/процессов/таймаутов).
    ...
    try:
        ...
        if should_debug_flag:
            logger.debug(f"Ошибка сборки конфига xray: {e}")
        if semaphore_acquired:
            _xray_checker_semaphore.release()
        return (vless_line, False, metrics)
    if XRAY_REUSE_WORKER:
        if tw.config_path is None:
            try:
                ...
                metrics["failure_type"] = "BUILD_CONFIG_ERROR"
                if should_debug_flag:
                    logger.debug(f"Не удалось создать файл конфига xray: {e}")
                if semaphore_acquired:
                    _xray_checker_semaphore.release()
                return (vless_line, False, metrics)
    ...
    try:
        ...
        if STRONG_STYLE_TEST:
            ...
            connect_t = max(
                STRONG_STYLE_CONNECT_T_MIN,
                min(10, int(STRONG_STYLE_TIMEOUT * 0.4)),
            )
            read_t = max(STRONG_STYLE_READ_T_MIN, STRONG_STYLE_TIMEOUT - connect_t)
            ...
            for attempt in range(attempts_needed):
                ...
                if response and not error and check_response_valid(response, 0, test_url):
                    if max_ok_time > 0 and elapsed_time > max_ok_time:
                        metrics["failure_type"] = "TIMEOUT_RESPONSE_TOO_SLOW"
                        if should_debug_flag:
                ...
                metrics["failure_type"] = "TIMEOUT_REQUEST_ERROR" if error else "INVALID_RESPONSE"
                unregister_process(proc, port)
                return (vless_line, False, metrics)
        ...
        if not all_urls:
            if TEST_URL:
                ...
                metrics["failure_type"] = "INVALID_TEST_CONFIG_NO_URLS"
                if should_debug_flag:
        ...
        # Для сводной диагностики всегда оставляем тип фейла, если ключ не прошёл проверку.
        if (not is_available) and not metrics.get("failure_type"):
            metrics["failure_type"] = "INVALID_RESPONSE"
        ...
        if XRAY_REUSE_WORKER and is_available:
            _preserve_xray_process = True
        else:
            unregister_process(proc, port)
        ...
        if should_debug_flag:
            ...
            _ep = proc or (tw.proc if XRAY_REUSE_WORKER else None)
            if _ep and _ep.stderr:
                try:
                    err = _ep.stderr.read().decode("utf-8", errors="replace")
                    if err.strip():
        ...
        # Важно: нельзя делать «голый» return в finally - в Python он подменяет return из try и вернёт None.
        _keep_alive = (
            XRAY_REUSE_WORKER
        ...
        if _keep_alive:
            ...
            if _cleanup_proc is not None:
                ...
                if should_debug_flag and _cleanup_proc.stderr is not None and _cleanup_proc.poll() is not None:
                    try:
                        err = _cleanup_proc.stderr.read().decode("utf-8", errors="replace")
                        if err.strip():
