...
# Strict style: минимальные таймауты на connect/read (в секундах).
# Важно: эти значения используются только для вычисления таймаут-пары
# (connect_t, read_t) при запросах строгого режима.
# Phase 1 может подрезать их отдельными env-параметрами, чтобы быстрее
# “выдыхать” заведомо плохие ключи.
STRONG_STYLE_CONNECT_T_MIN = _env_int("STRONG_STYLE_CONNECT_T_MIN", 3)
STRONG_STYLE_READ_T_MIN = _env_int("STRONG_STYLE_READ_T_MIN", 5)
STRONG_MAX_RESPONSE_TIME = _env_float("STRONG_MAX_RESPONSE_TIME", 3.0)
...
# Ограничение параллельных запусков xray внутри checker-а.
# По умолчанию семафор не ограничивает (предел = MAX_WORKERS), чтобы при мощном железе
# не терять производительность по сравнению с исходной схемой ThreadPoolExecutor.
# При необходимости снижения false negative/ошибок перегруза можно задать:
#   XRAY_CHECKER_MAX_CONCURRENCY=<число>
XRAY_CHECKER_MAX_CONCURRENCY = _env_int("XRAY_CHECKER_MAX_CONCURRENCY", 0)
if XRAY_CHECKER_MAX_CONCURRENCY <= 0:
    XRAY_CHECKER_MAX_CONCURRENCY = MAX_WORKERS
...
# Один процесс xray на поток ThreadPoolExecutor: тот же SOCKS-порт и файл конфига; на Unix при смене ключа
# перезагрузка конфига через SIGHUP (без fork/exec). Отключить: XRAY_REUSE_WORKER=false
XRAY_REUSE_WORKER = _env_bool("XRAY_REUSE_WORKER", True)
...
# Управляемая деградация (managed degradation) при типовых сбоях старта/сокета.
# Эти ошибки обычно транзиентные (перегруз/таймаут), поэтому им даём шанс повторить проверку,
# но не добавляем их в `notworkers` (чтобы не отфильтровывать навсегда).
TRANSIENT_RETRY_COUNT = _env_int("TRANSIENT_RETRY_COUNT", 1)
TRANSIENT_RETRY_BACKOFF_BASE_SEC = _env_float("TRANSIENT_RETRY_BACKOFF_BASE_SEC", 0.2)
TRANSIENT_RETRY_BACKOFF_MULT = _env_float("TRANSIENT_RETRY_BACKOFF_MULT", 1.5)
EXCLUDE_TRANSIENT_FROM_NOTWORKERS = _env_bool("EXCLUDE_TRANSIENT_FROM_NOTWORKERS", True)
...
# Минимальная доля/число успешных HTTP-запросов в latency-фазе speedtest.
# Конфиг считается валидным только если выполнены оба порога.
SPEED_TEST_MIN_SUCCESS_RATIO = _env_float("SPEED_TEST_MIN_SUCCESS_RATIO", 0.6)
SPEED_TEST_MIN_SUCCESS_COUNT = _env_int("SPEED_TEST_MIN_SUCCESS_COUNT", 5)
